...
def main() -> int:
    ...
    # === Step 3: Collect all label text files from TAGGED_BASE (now with subdirectories) ===
    text_paths: List[Path] = sorted(
        p
        for p in TAGGED_BASE.rglob("*.txt")
        if p.is_file()
    )

    if not text_paths:
        print(
            f"Error: no .txt files found in tagged base directory (including subdirectories): {TAGGED_BASE}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        with FILE_IDS.open("w", encoding="utf-8") as fidx:
            for i, text_file in enumerate(text_paths, 1):
                ...
                # Record path relative to TAGGED_BASE to preserve subcorpus (es/mg/rj/sp)
                rel_name = text_file.relative_to(TAGGED_BASE).as_posix()
    ...
    for text_file in text_paths:
        ...
        text_infos.append(
            {
                "id": fid,
                "name": text_file.relative_to(TAGGED_BASE).as_posix(),
                "labels": present,
            }
        )
