...
data scores_only (KEEP= filename state group &factorvars );
set scores_grouped ;
run;

/* fix variable order */
data scores_only;
 retain filename state group &factorvars;
 set scores_grouped;
run;
...
ods output OverallANOVA=overall_state_f&i ;
ods output FitStatistics=params_state_f&i ;
ods output ModelANOVA=anova_state_f&i ;
ods output Means=means_state_f&i ;

proc GLM data=scores;
    title GLM for dataset = &project state fac&i ;
    class state ;
    model fac&i = state ;
    means state ;
run;
...
PROC EXPORT
  DATA= WORK.overall_state_f&i
  DBMS=TAB
  OUTFILE="&whereisit/&myfolder/overall_state_f&i..tsv"
  REPLACE;
RUN;

PROC EXPORT
  DATA= WORK.params_state_f&i
  DBMS=TAB
  OUTFILE="&whereisit/&myfolder/params_state_f&i..tsv"
  REPLACE;
RUN;

PROC EXPORT
  DATA= WORK.anova_state_f&i
  DBMS=TAB
  OUTFILE="&whereisit/&myfolder/anova_state_f&i..tsv"
  REPLACE;
RUN;

PROC EXPORT
  DATA= WORK.means_state_f&i
  DBMS=TAB
  OUTFILE="&whereisit/&myfolder/means_state_f&i..tsv"
  REPLACE;
RUN;
...
cards4;
/home/u63529080/cl_st1_ph2_anna
;;;;
run;
